 ...
 *
 * الإصلاحات:
 * ✔ Bug 5: clearCanvas تحفظ الحالة الممسوحة في السجل لتمكين Redo بعد المسح
 */
...
// -------------------- مسح اللوحة مع حفظ صحيح في السجل --------------------
// Bug 5 Fix: نحفظ الحالة قبل المسح ثم الحالة بعد المسح، لتعمل Redo بشكل صحيح
...
function clearCanvas() {
    ...
    // حفظ الحالة الحالية (قبل المسح) في السجل
    saveToHistory(currentPage);

    // المسح الفعلي
    page.ctx.clearRect(0, 0, page.canvas.width, page.canvas.height);

    // Bug 5 Fix: حفظ الحالة الممسوحة أيضاً حتى يعمل Redo
    saveToHistory(currentPage);
