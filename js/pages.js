...
function createNewPage() {
    ...
    const pageData = {
        ...
        translateX: 0,
        translateY: 0,
        scale:      (typeof globalScale !== 'undefined') ? globalScale : 1,
        element:    pageElement,
    ...
    // تطبيق الـ scale الحالي على الصفحة الجديدة فور إنشائها
    if (typeof applyTransform === 'function') applyTransform(pageData);
    addPageToSidebar();
...
function deletePage(pageIndex) {
    ...
    // إزالة الـ wrapper والصفحة من DOM
    if (pages[pageIndex]) {
        // إذا كان للصفحة wrapper (تُنشئه zoom.js) نحذفه كاملاً
        if (pages[pageIndex].wrapper) {
            pages[pageIndex].wrapper.remove();
        } else if (pages[pageIndex].element) {
            // fallback: حذف عنصر الصفحة مباشرة إن لم يكن هناك wrapper
...
function switchToPage(pageIndex) {
    ...
    if (typeof applyTransformAll === 'function') applyTransformAll();
    else if (typeof applyTransform === 'function') applyTransform(targetPage);
