...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleTouchStart = (e) => {
        ...
        if (e.touches.length > 1) {
            ...
            // Bug 7 Fix: وقف الحدث حتى لا يصل لـ viewport-lock.js
            if (e.touches.length === 2) {
                ...
                const area = _getCanvasArea();
                if (area) {
                    const rect = area.getBoundingClientRect();
                    pinchCenter = {
                        x: (t1.clientX + t2.clientX) / 2 - rect.left + area.scrollLeft,
                        y: (t1.clientY + t2.clientY) / 2 - rect.top  + area.scrollTop
                    };
                }
            }
    ...
    const handleTouchMove = (e) => {
        ...
        // ── Bug 2 Fix: Pan بإصبع واحد → scroll تتولاه viewport-lock.js ──
        // ── Pinch بإصبعين → zoom على الصفحة ──
        if (isPanning) {
            if (e.touches.length === 2) {
                // Bug 7 Fix: stopPropagation حتى لا يتداخل مع viewport-lock
                e.stopPropagation();
                ...
                if (pinchStartDistance > 0) {
                    ...
                    page.scale     = Math.max(0.2, Math.min(initialScale * factor, 5));

                    if (typeof applyTransform === 'function') applyTransform(page);

                    const scaleRatio = page.scale / oldScale;
                    area.scrollLeft  = pinchCenter.x * scaleRatio - (
                        (t1.clientX + t2.clientX) / 2 - area.getBoundingClientRect().left
                    );
                    area.scrollTop   = pinchCenter.y * scaleRatio - (
                        (t1.clientY + t2.clientY) / 2 - area.getBoundingClientRect().top
                    );
                }
            }
            // إصبع واحد في Pan: الـ scroll تتولاه viewport-lock.js بالكامل
            return;
        }
